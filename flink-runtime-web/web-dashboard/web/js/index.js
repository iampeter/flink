var process = {env: {"NODE_ENV": "development", jobServer: "http://localhost:8081/"}};
angular.module('flinkApp', ['ui.router', 'angularMoment', 'dndLists']).run(["$rootScope", function($rootScope) {
  ...
  jobServer: process.env.jobServer,
        ...
        pr = _this.getMetrics(jid, node.id, metricIds).then(function(metrics) {
          return angular.forEach(_.keys(metrics.values), (function(_this) {
            return function(key) {
              ...
              } else if ((key.indexOf("numBytesInLocalPerSecond") !== -1 || key.indexOf("numBytesInRemotePerSecond") !== -1) && _.contains(sinks, node.id)) {
